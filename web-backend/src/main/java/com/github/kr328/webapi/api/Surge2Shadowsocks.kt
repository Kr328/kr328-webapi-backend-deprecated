...
import com.github.kr328.webapi.api.serializer.ShadowsocksSerializer
import com.github.kr328.webapi.api.serializer.SurgeSerializer
...
fun surge2Shadowsocks(data: String, extras: Map<String, String?>): String {
    val proxy = SurgeSerializer().parse(data).proxy.filterIsInstance<Shadowsocks>()
    val shadowsocksD = ShadowsocksD(provider = extras[EXTRA_SHADOWSOCKS_D_PROVIDER_NAME] ?: "Unlabeled",
            ...
            servers = proxy,
            trafficUsed = extras[EXTRA_SHADOWSOCKS_D_TRAFFIC_USED]?.toLong(),
            trafficTotal = extras[EXTRA_SHADOWSOCKS_D_TRAFFIC_TOTAL]?.toLong(),
            url = null,
            expires = null)

    return ShadowsocksSerializer().stringify(shadowsocksD)
}
