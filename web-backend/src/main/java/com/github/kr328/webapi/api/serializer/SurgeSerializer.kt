...
class SurgeSerializer {
    ...
    fun parse(string: String): Surge {
        ...
        loop@ for (line in string.split('\r', '\n').map(String::trim)) {
            when {
                            ...
                            Stream.of(line)
                                    ...
                                    .forEach {
                                        proxyGroup[it[0]] = when (it[1]) {
                                            ...
                                            "url-test" ->
                                                parseProxyGroupArgs(it.subList(2, it.size)).let { m ->
                                                    @Suppress("UNCHECKED_CAST")
                                                    ProxyGroup(ProxyGroup.TypeUrlTest(m["url"].toString(),
                                            ...
                                            "fallback" ->
                                                @Suppress("UNCHECKED_CAST")
                                                parseProxyGroupArgs(it.subList(2, it.size)).let { m ->
                                                    ProxyGroup(ProxyGroup.TypeFallback(m["url"].toString(),
                                                            m["interval"]?.toString()?.toLong()),
                                                            m["proxies"] as List<String>)
                                                }
