...
public class Surge2ShadowSocks {
    ...
    public Mono<ServerResponse> process(ServerRequest request) {
        ...
        String decodedUrl = new String(Base64.getUrlDecoder().decode(url.get().toString()));
    ...
    private String buildForResponse(HttpHeaders httpHeaders ,Proxy[] proxies) {
        ...
        } catch (BaseSubscription.BuildException e) {
            throw new WebServerException("build SSD config failure" ,e);
        }
    }
}
